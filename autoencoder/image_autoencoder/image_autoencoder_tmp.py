...
def ae_loss(y, yhat, disc_pior_fake_output):
    rec_loss = mse_loss(y, yhat)
    disc_prior_fake_loss = bce_loss(disc_pior_fake_output, torch.ones_like(disc_pior_fake_output).to(device))
    
    total_loss = 1.0 * rec_loss + 0.5 * disc_prior_fake_loss
    return total_loss
    

# discriminator prior loss function
def disc_loss(disc_real_output, disc_fake_output):
    real_loss = bce_loss(disc_real_output, torch.ones_like(disc_real_output).to(device))
    fake_loss = bce_loss(disc_fake_output, torch.zeros_like(disc_fake_output).to(device))
...
discPrior_optimizer = torch.optim.Adam(disc_prior.parameters(), lr=5e-3)
...
def plot_ae_outputs(encoder, decoder, epoch, n=5):
    ...
    for i in range(n):
      ax = plt.subplot(2,n,i+1)
      img = full_dataset[i][0].unsqueeze(0).to(device)
...
for epoch in range(epochs):
    ...
    discPrior_epoch_loss = 0
    discImage_epoch_loss = 0
    ...
    for batch_features, _ in train_dataloader:
        ...
        discPrior_optimizer.zero_grad()
        
        with torch.no_grad():
            fake_output = encoder(batch_features) 
        real_output = sample_normal(fake_output.shape)
    
        discPrior_real_output =  disc_prior(real_output)
        discPrior_fake_output =  disc_prior(fake_output)   
        
        discPrior_loss = disc_loss(discPrior_real_output, discPrior_fake_output)
        
        discPrior_loss.backward()
        discPrior_optimizer.step()
        
        discPrior_epoch_loss += discPrior_loss.item()
        ...
        discPrior_fake_output = disc_prior(encoded_images)
            
        autoencoder_loss = ae_loss(batch_features, reconstructed_images, discPrior_fake_output)
        
        autoencoder_loss.backward()
        ae_optimizer.step()
        
        autoencoder_epoch_loss += autoencoder_loss.item()
    
    # compute the epoch training loss
    discPrior_epoch_loss = discPrior_epoch_loss / len(train_dataloader)
    discImage_epoch_loss = discImage_epoch_loss / len(train_dataloader)
    ...
    print("epoch : {}/{}, dp_loss = {:.6f} ae_loss = {:.6f}, time = {:.2f}".format(epoch + 1, epochs, discPrior_epoch_loss, autoencoder_epoch_loss, (tock - tick)))
...
encoder_weights_file = "aae_encoder_weights_ballroom_epoch_100_cpu"
decoder_weights_file = "aae_decoder_weights_ballroom_epoch_100_cpu"
discPrior_weights_file = "aae_discPrior_weights_ballroom_epoch_100_cpu"

# load model weights
encoder.load_state_dict(torch.load(encoder_weights_file))
decoder.load_state_dict(torch.load(decoder_weights_file))
discPrior.load_state_dict(torch.load(discPrior_weights_file))

# save model weights
torch.save(encoder.state_dict(), encoder_weights_file)
torch.save(decoder.state_dict(), decoder_weights_file)
torch.save(discPrior.state_dict(), discPrior_weights_file)
...
# get two example images (and their labels) from the test dataset
img_1, label_1 = dataset[0]
img_2, label_2 = dataset[1]

# show the two example images
plt.imshow(np.moveaxis(img_1.numpy(), 0, 2))
plt.imshow(np.moveaxis(img_2.numpy(), 0, 2))
...
decoder.eval()
with torch.no_grad():
    rec_img_1 = decoder(encoder(img_1.to(device)))
    rec_img_2 = decoder(encoder(img_2.to(device)))
encoder.train()
decoder.train()

rec_img_1 = rec_img_1.cpu().detach().numpy().squeeze()
rec_img_2 = rec_img_2.cpu().detach().numpy().squeeze()

plt.imshow(np.moveaxis(rec_img_1, 0, 2))
plt.imshow(np.moveaxis(rec_img_2, 0, 2))

# mix image codes

img_encoding_1 = encoder(img_1.to(device))
img_encoding_2 = encoder(img_2.to(device))

mix_factor = 0.5

mixed_encoding = img_encoding_1 * mix_factor + img_encoding_2 * (1.0 - mix_factor)

mixed_img = decoder(mixed_encoding)

mixed_img = mixed_img.cpu().detach().numpy().squeeze()

plt.imshow(np.moveaxis(mixed_img, 0, 2))

# create vector interpolation animation
...
for mix_factor in np.linspace(0.0, 1.0, 100):
    mixed_encoding = img_encoding_1 * mix_factor + img_encoding_2 * (1.0 - mix_factor)
    ...
    #mixed_img = np.clip(mixed_img, 0.0, 1.0)
    mixed_img = np.moveaxis(mixed_img, 0, 2)
    
    plt.imshow(mixed_img)
    plt.savefig("mixed_{0:05d}.jpg".format(mix_index))
...
parameter:
    image_indices: indices of images to display
    dataset: dataset to retrieve images from
"""
def draw_images(image_indices, dataset):
    index_count = len(image_indices)
    rows = int(math.ceil(math.sqrt(index_count)))
    columns = int(math.ceil(index_count / rows))
    ...
    for row in range(rows):
        for col in range(columns):
            ...
            img, _ = dataset[image_indices[image_nr]]
            img = img.squeeze()
            
            # Adds a subplot at the 1st position
            fig.add_subplot(rows, columns, image_nr + 1)
            
            # showing image
            plt.imshow(np.moveaxis(img.numpy(), 0, 2))
            plt.axis('off')
            plt.title(image_indices[image_nr])
            plt.tight_layout()
...
"""
    creates a list of image encodings 
    parameter: 
        image_indices : indices of images to encode
        dataset: dataset to load the images from
"""
def encode_images(image_indices, dataset):
    ...
    for image_index in image_indices:
            ...
            img_enc = encoder(img.unsqueeze(0).to(device))
            
            img_enc = img_enc.cpu().detach().numpy().squeeze()
...
"""
save image encodings as "pickled" file
parameter: 
    image_encodings: numpy array of image encodings
    file_name: name of file (ideally ends on ".p")
"""
def save_image_encodings(image_encodings, file_name):
    with open(file_name, 'wb') as file:
        pickle.dump(image_encodings, file)

# select images
selected_image_indices = [0, 5, 7, 10, 12]

# display selected images
draw_images(selected_image_indices, dataset)

# encode selected images
image_encodings = encode_images(selected_image_indices, dataset)

# save encodings of selected images
save_image_encodings(image_encodings, "ballroom_encodings.p")
